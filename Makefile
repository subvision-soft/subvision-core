...
LIB_SOURCES = native/src/utils.cpp \
			native/src/image_processing.cpp \
			native/src/target_detection.cpp \
			native/src/impact_detection.cpp \
			native/src/sheet_detection.cpp \
...
$(OUTPUT_DIR)/subvision.js: $(OUTPUT_DIR)
	docker run --rm -v "$(CURDIR):/src" -w /src $(DOCKER_IMAGE) bash -c "emcc $(LIB_SOURCES) wasm/emscripten_binding.cpp \
		-I./native/include \
...
$(OUTPUT_DIR)/index.html: $(OUTPUT_DIR) wasm/web/index.html
	cp wasm/web/index.html $(OUTPUT_DIR)/
...
ifeq ($(OS),Windows_NT)
	...
	@cd build-dotnet-x64 && cmake -G "Visual Studio 17 2022" -A x64 -DBUILD_CLI_WRAPPER=ON -DBUILD_TESTS=OFF ..
...
ifeq ($(OS),Windows_NT)
	...
	@cd build-dotnet-arm64 && cmake -G "Visual Studio 17 2022" -A ARM64 -DBUILD_CLI_WRAPPER=ON -DBUILD_TESTS=OFF ..
...
ifeq ($(OS),Windows_NT)
	...
	@copy dotnet\Subvision.nuspec nupkg\
	@copy dotnet\Subvision.targets nupkg\
